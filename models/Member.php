...
class Member extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['chinese_zodiac', 'religion', 'horoscope',  'previous_employment', 'profile_photo'], 'default', 'value' => null],
            [['work_experience_years'], 'default', 'value' => 0],
            [['status'], 'default', 'value' => 'AVAILABLE'],
            [['languages', 'skills'], 'safe'],
            ...
            [['gender', 'marital_status',  'previous_employment', 'status'], 'string'],
    ...
    {
        return ['Cantonese', 'English', 'Mandarin', 'Tagalog', 'Bahasa Indonesia'];
    }

    public static function getUniqueNationalities()
    {
        return self::find()->select('nationality')->distinct()->column();
    }

    public function getSkills()
    {
        return is_array($this->skills) ? $this->skills : ($this->skills && is_string($this->skills) ? json_decode($this->skills, true) : []);
    }

    public function getLanguages()
    {
        return is_array($this->languages) ? $this->languages : ($this->languages && is_string($this->languages) ? json_decode($this->languages, true) : []);
    }
    ...
    public function getSkillsArray()
    {
        ...
        // 解析 skills
        $skills = is_array($this->skills) ? $this->skills : ($this->skills && is_string($this->skills) ? json_decode($this->skills, true) : []);
        \Yii::debug("Raw skills: " . print_r($this->skills, true), __METHOD__);
        \Yii::debug("Parsed skills: " . print_r($skills, true), __METHOD__);

        $skillsMap = [];
        if (is_array($skills)) {
            $skillsMap = array_column($skills, 'value', 'skill');
        }
        \Yii::debug("Skills map: " . print_r($skillsMap, true), 'skills_map');
